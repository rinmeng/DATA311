...
3.  **Residual Standard Error**: The residual standard error has slightly decreased from $2.732$ in the linear model to \$\$ in the polynomial model, indicating a better fit.
4.  **Violations:**
...
-   Sex = I is highly significant with a p-value of $2.07 \times 10^{-16}$.

-   Sex = M is insignificant with a p-value of $0.348$.

-   **Model with only Length:** Adjusted R-squared = $0.2843$
...
For this forward selection model, I have selected `Whole_Weight` which is the `Whole_weight` variable where it measures the whole abalone's we
...
The **interaction term** `X0.445:Whole_Weight` has a very low p-value at $2 \times 10^{-16}$, making it statistically significant. The negative coefficient of the interaction term suggests that, as the variable **Whole Weight** increases, the effect of **Length** on the number of **Rings** decreases.
...
2.  **Diameter (Diameter)**: p-value $= 0.00777$

3.  **Height (Height)**: p-value $= 4.71 \times 10^{-06}$

4.  **Whole_Weight (Whole Weight)**: p-value = $2.77 \times 10^{-16}$

5.  **Shucked_Weight (Shucked Weight)**: p-value $< 2 \times 10^{-16}$

6.  **Viscera_Weight (Viscera Weight)**: p-value $= 1.72 \times 10^{-08}$

7.  **Shell_Weight (Shell Weight)**: p-value $= 2.75 \times 10^{-09}$
...
2.  **Length Length**: The p-value is 0.61741, which is also not statistically significant, and also, it is highly correlated with the **Diameter**.
...
```{r}
myModel <- lm(Rings ~ Sex + Diameter + Height + Whole_Weight + Shucked_Weight + Viscera_Weight + Shell_Weight, data = abaloneTrain)
summary(myModel)
...
I chose this model because I excluded the variables that were not statistically significant, such as the variable **Length Length.**\

This model performed a little bit better than the all of the other models because the **Adjusted R-squared** is up by $0.0002$

## Model Comparison

### Exercise 22

| Exercise | Adjusted $R^2$                                             | $RSE_{test}$                                       | $MSE_{test}$                                                                                      |
|-----------------|-----------------|-----------------|----------------------|
...
| 10       | `r summary(abaloneTrain.lm2)$adj.r.squared`                | `r summary(abaloneTrain.lm2)$sigma`                | `r mean((predict(abaloneTrain.lm2, newdata = abaloneTest) - abaloneTest$Rings)^2)`                |
| 11       | `r summary(abaloneTrain.lm3)$adj.r.squared`                | `r summary(abaloneTrain.lm3)$sigma`                | `r mean((predict(abaloneTrain.lm3, newdata = abaloneTest) - abaloneTest$Rings)^2)`                |
| 15       | `r summary(abaloneTrain.lm3withWholeWeight)$adj.r.squared` | `r summary(abaloneTrain.lm3withWholeWeight)$sigma` | `r mean((predict(abaloneTrain.lm3withWholeWeight, newdata = abaloneTest) - abaloneTest$Rings)^2)` |
| 17       | `r summary(abaloneTrain.lm4)$adj.r.squared`                | `r summary(abaloneTrain.lm4)$sigma`                | `r mean((predict(abaloneTrain.lm4, newdata = abaloneTest) - abaloneTest$Rings)^2)`                |
| 18       | `r summary(interactionModel)$adj.r.squared`                | `r summary(interactionModel)$sigma`                | `r mean((predict(interactionModel, newdata = abaloneTest) - abaloneTest$Rings)^2)`                |
| 19       | `r summary(multipleRegression)$adj.r.squared`              | `r summary(multipleRegression)$sigma`              | `r mean((predict(multipleRegression, newdata = abaloneTest) - abaloneTest$Rings)^2)`              |
| 21       | `r summary(myModel)$adj.r.squared`                         | `r summary(myModel)$sigma`                         | `r mean((predict(myModel, newdata = abaloneTest) - abaloneTest$Rings)^2)`                         |
